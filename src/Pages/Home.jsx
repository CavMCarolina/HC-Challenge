import Footer from "../Componentes/Footer/Footer"
import bg from "../assets/background.jpg"

export default function Main() {
    return (
        <>
            <main>
                <hr className="hr-normal"/>
                <img src={bg} alt="Foto de fundo"/>
                <div className="main_container">
                    <div className="home">
                        <iframe id="video_home" src="https://www.youtube.com/embed/dfaM_jT1LkQ?si=88rT5f0Vo1VTrZ0u" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen=""></iframe>
                        <p>
                            A infância é um momento muito importante para a vida da criança. Contudo, essa fase, para algumas crianças, pode ser marcada por desafios, momentos difíceis e muitas vezes os pais e as crianças se encontram ansiosos e perdidos em relação aos procedimentos médicos. Pensando nisso desenvolvemos um site para o Hospital das Clínicas para facilitar o acesso dos pacientes aos exames e procedimentos.
                            <br/><br/>
                            É um ambiente virtual, acolhedor e informativo, direcionado às necessidades das crianças, pais e acompanhantes através de uma interface amigável e intuitiva.
                            <br/><br/>
                            Além de contar com as informações que já existiam no site, nossa equipe utilizou tecnologias mais atuais para criar o Espaço Interativo. Por meio dele, nosso site oferece acesso prático a informações relevantes sobre o hospital, seus serviços e os procedimentos realizados de uma forma mais visual, quebrando a barreira do analfabetismo ou de outras limitações pessoais.
                        </p>
                    </div>
                    {/* <div>
                        <div>
                            <h1>Sumário:</h1>
                            <ul>
                                <li><a href="#exames">Exames</a></li>
                                <li><a href="#agendamentos">Agendamentos</a></li>
                                <li><a href="#voluntarios">Voluntários</a></li>
                                <li><a href="#icr">Instituto da Crinça (Icr)</a></li>
                            </ul>
                        </div>
                        <section id="exames">
                            <h2>Exames</h2>
                            <div>
                                <img src=""/>
                                <p>TESTE</p>
                            </div>
                        </section>
                        <section id="agendamentos">
                            <h2>Agendamentos</h2>
                            <div>
                                <img src=""/>
                                <p>TESTE</p>
                            </div>
                        </section>
                        <section id="voluntarios">
                            <h2>Voluntários</h2>
                            <div>
                                <img src=""/>
                                <p>TESTE</p>
                            </div>
                        </section>
                        <section id="icr">
                            <h2>ICr</h2>
                            <div>
                                <img src=""/>
                                <p>TESTE</p>
                            </div>
                        </section>
                    </div> */}
                </div>
            </main>
            <Footer/>
        </>
    )
}